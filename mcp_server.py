...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Actors are components (entities at OA) flagged is_actor: true -- capellambse has no "
        "separate actor class, so type alone looks identical. Browse object_type='Actor' to list "
        "only actors; 'Component' still includes them. Don't treat two similarly named components "
        ...
        "Diagram browsing IS scoped to the phase you pass. A diagram's phase is determined from "
        "the layer containing the element it targets -- NOT from its 'viewpoint' field, which is "
        "the Sirius viewpoint and groups diagram KINDS rather than Arcadia layers: every "
        "functional-chain, sequence, state-machine and class diagram reports viewpoint 'Common' "
        "whichever layer it was drawn in (26 of 88 diagrams on a real model, spanning SA, LA and "
        "PA together). So do not read 'viewpoint' as a layer -- read the 'layer' field, which "
        "each diagram result carries and which reports the real one (OA/SA/LA/PA/EPBS). "
        "The only diagrams shown under every phase are those belonging to none: EPBS diagrams, "
        "and any whose target cannot be resolved. Those appear everywhere rather than being "
        "hidden from all four, so no diagram is ever unreachable. "
