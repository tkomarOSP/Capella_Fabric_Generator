...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Diagram browsing IS scoped by the phase you pass, but deliberately not strictly: you "
        "get that phase's own diagrams, PLUS cross-cutting ones that belong to no phase -- CDB "
        "(Class Diagram Blank / data) diagrams, whose viewpoint is 'Common' and which describe "
        "data used everywhere, and any viewpoint with no browse phase of its own (e.g. 'EPBS "
        "architecture'). Those cross-cutting diagrams appear under every phase rather than being "
        "hidden from all of them, which is what happened when capellambse's per-layer diagram "
        "accessors were used (each filters by a fixed viewpoint string, not true layer-scoping). "
        "Every diagram result carries a 'viewpoint' field saying which it is, so a Common diagram "
        "is distinguishable from the phase's own -- check it rather than assuming a result under "
        "phase PA is a PA diagram. "
